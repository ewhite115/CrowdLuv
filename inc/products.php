...
function get_cl_user_followed_talent($fbid) {

    require(ROOT_PATH . "inc/database.php");

    try {
        $sql = "SELECT * FROM follower join follower_luvs_talent join talent on follower.crowdluv_uid = follower_luvs_talent.crowdluv_uid and follower_luvs_talent.crowdluv_tid = talent.crowdluv_tid where follower.fb_uid=? LIMIT 0, 30 ";
        $results = $db->prepare($sql);
        $results->bindParam(1,$fbid);
        $results->execute();
    } catch (Exception $e) {
